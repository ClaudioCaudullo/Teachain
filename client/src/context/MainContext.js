...
export const MainProvider = ({ children }) => {
  ...
  useEffect(() => {
    if (contractUserDetails && Object.keys(contractUserDetails).length != 0)
      loadUserDetails();
  }, [contractUserDetails]);
  ...
  const loadCourses = async () => {
    ...
    try {
      ...
      let fetchCourses = await Promise.all(
        results.map(async (i) => {
          if (!i || i.hash == "-1") return;
          let response = await fetch(
            `https://learningdata.infura-ipfs.io/ipfs/${i.hash}`
          );
          ...
          let nPurchase = await contractCourses.getNumberPurchase(
            course.content.id
          );
  ...
  async function loadBuyedCourses() {
    ...
    let vector = courses.map((course) => {
      for (let i = 0; i < course.content.nPurchase; i++)
        if (
          course.content.buyers[i].toLowerCase() == currentAccount.toLowerCase()
        ) {
          return course;
        }
    });
  ...
  async function loadUserReviews(userid) {
    ...
    let fetchReviews = await Promise.all(
      results.map(async (i) => {
        if (!i || !i.hash || i.hash == "-1") return;
        let response = await fetch(
          `https://learningdata.infura-ipfs.io/ipfs/${i.hash}`
  ...
  const loadUserDetails = async () => {
    ...
    if (!results || results == "") {
      ...
      let data = await fetch(
        `https://learningdata.infura-ipfs.io/ipfs/${results}`
      );
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      let data = await fetch(
        `https://learningdata.infura-ipfs.io/ipfs/${productHash}`
      );
      data = await data.json();
      setUsername(data.userDetails.username);
      setEmail(data.userDetails.email);
