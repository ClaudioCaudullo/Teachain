...
const Home = () => {
  ...
  useEffect(() => {
    ...
    window.addEventListener("scroll", () => {
      ...
      if (
        document.getElementById("immaginiPubblicita2") &&
        isScrolledIntoView(document.getElementById("immaginiPubblicita2"))
      ) {
        document
          .getElementById("immaginiPubblicita2")
          .classList.add(styles.fadeInRight);
      }
      if (
        document.getElementById("scrittaPubblicita2") &&
        isScrolledIntoView(document.getElementById("scrittaPubblicita2"))
      ) {
        document
          .getElementById("scrittaPubblicita2")
          .classList.add(styles.fadeInLeft);
      }

      if (
        document.getElementById("immaginiPubblicita3") &&
        isScrolledIntoView(document.getElementById("immaginiPubblicita3"))
      ) {
        document
          .getElementById("immaginiPubblicita3")
          .classList.add(styles.slideIn);
      }
      if (
        document.getElementById("scrittaPubblicita3") &&
        isScrolledIntoView(document.getElementById("scrittaPubblicita3"))
      ) {
        document
          .getElementById("scrittaPubblicita3")
          .classList.add(styles.bounceInBottom);
        document.getElementById("scrittaPubblicita3").style.opacity = "1";
      }

      if (
        document.getElementById("immaginiPubblicita4") &&
        isScrolledIntoView(document.getElementById("immaginiPubblicita4"))
      ) {
        document
          .getElementById("immaginiPubblicita4")
          .classList.add(styles.slideIn);
      }
      if (
        document.getElementById("scrittaPubblicita4") &&
        isScrolledIntoView(document.getElementById("scrittaPubblicita4"))
      ) {
        document
          .getElementById("scrittaPubblicita4")
          .classList.add(styles.bounceInBottom);
  ...
  function applyFilters() {
    setCoursesCopy(
      courses.map((course) => {
        if (
          course.content.subject.includes(filterSubject) ||
          (course.content.secondary &&
            course.content.secondary.includes(filterSubject))
        )
          if (course.content.price <= value) return course;
      })
    );
  }
  ...
  const uploadUserDetails = async () => {
    ...
    await (
      await contractUserDetails
        .addUser(currentAccount, userDetails.username, userDetails.email, hash)
        .catch((error) => {
          setAlertTextCustom("Registrazione fallita: ", error.code);
          setShow(true);
          setHomeLoading(false);
      ...
      {openModalRegistration ? (
        <>
          <Modal show={show} onHide={() => disconnect()}>
            ...
            <Modal.Body>
              Woohoo, scembra che tu sia un nuovo utente!!Parlaci un po di te
            </Modal.Body>
            ...
            <Modal.Body>
              ...
              <input
                className="form-control mb-4"
                type="email"
                placeholder="claudiocaudullo01@gmail.com"
            ...
            <Modal.Body>
              ...
              <textarea
                ...
                placeholder="Ciao, sono uno studente universitario appassionato di informatica..."
              />
            </Modal.Body>
            <Modal.Body>
              Sfondo profilo (opzionale):{" "}
              <input type="file" name="file" onChange={changeBackground} />
            </Modal.Body>
            <Modal.Body>
              Foto profilo (optional):{" "}
              ...
              <>
                      ...
                      <Card.Body>
                        ...
                        <div className={styles.containerSubject}>
                          <select
                            ...
                            id="materia"
                            onChange={(event) =>
                              setFilterSubject(event.currentTarget.value)
                            }
                          >
                            ...
                            <option value="P.E (Physical Education)">
                ...
                <>
                  {coursesCopy.map((course) => {
                    ...
                    if (!keyWord)
                      return (
                        <Card2 key={course.content.id} item={course.content} />
                      );
                    else if (
        ...
        <>
          <div className={styles.textCenter}>
            <main style={{ padding: "1rem 0" }}>
              <div className={styles.containerAds}>
                <div
                  className={`${styles.textAds} ${styles.fadeInLeft}`}
                  id="scrittaPubblicita1"
                >
                  <h2>
                    Trova fantastici insegnanti da tutte le parti del mondo!
                  </h2>
                </div>
          ...
          <div className={styles.textCenter}>
            <main style={{ padding: "1rem 0" }}>
              <div className={styles.containerAds}>
                <div
                  className={`${styles.textAds}`}
                  id="scrittaPubblicita2"
                  style={{ opacity: 0 }}
                >
                  <h2>Massima distanza, massima resa!</h2>
                  <h4>
                    Segui le lezioni degli insegnanti sorseggiando un caffè
                    comodamente da casa tua
                  </h4>
                </div>
                <div
                  className={`${styles.imageAds}`}
                  id="immaginiPubblicita2"
                  style={{ opacity: 0 }}
                >
              ...
              <div className={styles.containerAdsV2}>
                <div
                  className={`${styles.textAdsV2}`}
                  id="scrittaPubblicita3"
                  style={{ opacity: 0 }}
                >
                  <h2>Non un monopolio, ma una decentralizzazione</h2>
                  <h4>
                    Il progetto che si compone dei propri utenti! Non esiste un
                    punto centrale, tutti fanno parte della stessa catena
                  </h4>
                </div>
                <div
                  className={`${styles.imageAdsV2}`}
                  id="immaginiPubblicita3"
                  style={{ opacity: 0 }}
                >
                  <img
                    src={require("../images/decentralizzazione-transformed.jpeg")}
                  />
                </div>
              </div>
