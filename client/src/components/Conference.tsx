...
const Conference: React.FC = () => {
  ...
  useEffect(() => {
    socket.on(
      "new-user-arrived",
      (peerID: string, roomID: string, userName: string) => {
        imconnected = true;
        if (myStream) {
          setPeersArray((peers) => {
          ...
          call.on("stream", function (remoteStream: any) {
            setVideoStreams((streams) => {
              ...
              const found = streamsCopy.some(
                (el) => el.stream.id === remoteStream.id
              );
              remoteStream["peerID"] = peerID;
              if (!found)
                streamsCopy.push({ user: userName, stream: remoteStream });
              return streamsCopy;
            });
          });
        }
    ...
    myPeer.on("call", function (call) {
      if (myStream) {
        ...
        setPeersArray((peers) => {
          ...
          if (!found && call.peer !== myPeerUniqueID)
            streamsCopy.push(call.peer);
          return streamsCopy;
        });
        ...
        call.on("stream", function (remoteStream) {
          if (call.metadata.screenShare) {
            ...
            setVideoStreams((streams) => {
              ...
              if (!found && call.metadata.streamID !== remoteStream.id)
                streamsCopy.push({
                  user: call.metadata.userName,
                  stream: remoteStream,
                });
              return streamsCopy;
            });
  ...
  const startScreenShare = useCallback(async () => {
    try {
      ...
      let captureStream = await mediaDevices.getDisplayMedia({
        video: true,
        audio: false,
      });
  ...
  const videoStreamsList = useMemo(() => {
    for (let stream of videoStreams) {
      if (stream.stream["peerID"] && stream.stream["id"])
        arrayClient[stream.stream["peerID"]] = stream.stream["id"];
    }
    return (
      <>
        {myStream && (
      ...
      {roomId && (
        <div className="AppCover">
          <div className="video-streams">
            <div className="streamsCover">{videoStreamsList}</div>
            {videoSharingBlock}
          </div>
          <div className="video-sidebar">
            <div className="room-headline">
              <div>
                Copy and share the room ID in order to join the conference
              </div>
            ...
            {videoStreams.length > 0 && (
              <div className="screenShareCover">
                <button
                  ...
                  disabled={startSharingButtonDisabled ? true : false}
