...
const CoursesPurchased = () => {
  ...
  function applyFilters() {
    setCoursesCopy(
      buyedCourses.map((courses) => {
        if (
          courses.content.subject.includes(subjectFilter) ||
          (courses.content.secondary &&
            courses.content.secondary.includes(subjectFilter))
        )
          if (courses.content.price <= value) return courses;
      })
    );
  }
          ...
          {isConnected ? (
            <>
              {coursesCopy.map((courses) => {
                ...
                if (!key)
                  return (
                    <Card2 key={courses.content.id} item={courses.content} />
                  );
                else if (
