...
const Profilo2 = () => {
  ...
  let {isConnected,loadExternalUserData,setHash,currentAccount,userCourses,loadUserCourses,loadUserReviews,contractReviews,reviews,contractUserDetails,setProfileImage }=useContext(MainContext);
...
const localizer = dateFnsLocalizer({
  ...
  const uploadReview = async () => {
    ...
    await (await contractReviews.addReview(hash,id.userid).catch((err)=>{
  ...
  async function saveData(){
    ...
    const newReview={creator:currentAccount,title:title,text:text,vote:vote,reviewed:id.userid};
    setReview(newReview)
    }
}
...
useEffect(()=>{
  ...
  const fetchData=async()=>{
    let numReviews=0,totalVote=0;
  reviews.map((review)=>{
    if(review) {
    if(review.content.creator==currentAccount && review.content.reviewed==id.userid) {
  ...
  useEffect( ()=>{
    if(currentAccount) {
      ...
      await loadUserReviews(id.userid);
      if(hashExternalUser){
      let response=await fetch(`https://learningdata.infura-ipfs.io/ipfs/${hashExternalUser}`)
  ...
  useEffect(() => {
    ...
    async function confirmModifyBackgroundImage(){      
      if(!currentAccount || !username || !email || !localProfileImage || !fileBackgroundURL)
      {
      ...
      let newProfile={id:currentAccount,username:username,email:email,img:localProfileImage,background:fileBackgroundURL,description:description,disponibility:allEvents}
      setUserDetails(newProfile)
    }

    async function confirmModifyProfileImage(){   
      if(!currentAccount || !username || !email || !fileImageURL || !backgroundImage)
      {
      ...
      setFlipImage(true)
      let newProfile={id:currentAccount,username:username,email:email,img:fileImageURL,background:backgroundImage,description:description,disponibility:allEvents}
      setUserDetails(newProfile)
    }

    async function confirmModifyUsername(){
      if(!currentAccount || !newUsername || !email || !localProfileImage || !backgroundImage)
      {
      ...
      let newProfile={id:currentAccount,username:newUsername,email:email,img:localProfileImage,background:backgroundImage,description:description,disponibility:allEvents}
      setUserDetails(newProfile)
    }

    async function confirmModifyEmail(){
      if(!currentAccount || !username || !email || !newEmail || !backgroundImage || !newDescription)
      {
      ...
      let newProfile={id:currentAccount,username:username,email:newEmail,img:localProfileImage,background:backgroundImage,description:description,disponibility:allEvents}
      setUserDetails(newProfile)
    }
    ...
    async function confirmModifyDescription(){
      ...
      let newProfile={id:currentAccount,username:username,email:email,img:localProfileImage,background:backgroundImage,description:newDescription,disponibility:allEvents}
      setUserDetails(newProfile)
    }
    ...
    async function modifyReview()
    {
      ...
      setLoading("true")
      const review={creator:currentAccount,title:title,text:text,vote:vote,reviewed:id.userid};
      let hash
      try {
          let options={
              warpWithDirectory:true,
      ...
      await (await contractReviews.changeReview(hashReview,hash).catch((err)=>{
        setAlertText(err.code)
        setOpen(true)
        setSeverity("error")
        setLoading("false")
    ...
    {
      setLoading("true")
      await (await contractReviews.deleteReview(idReview).catch((err)=>{
        setAlertText(err.code)
        setOpen(true)
        setSeverity("error")
        setLoading("false")})).wait()
    ...
    function insertDate()
    {
      ...
      if(newDate) 
      {       
        var x=newDate.map((newDate)=>{if(newDate.start==data.start || newDate.end==data.end) {return 0}})
    ...
    function confirmBooking()
    {
      ...
      let newProfile={id:id.userid,username:username,email:email,img:localProfileImage,background:backgroundImage,description:description,disponibility:support}
      setUserDetails(newProfile)
    }
    ...
    function removeDisponibility()
    {
      ...
      let newProfile={id:currentAccount,username:username,email:email,img:localProfileImage,background:backgroundImage,description:description,disponibility:newDisponibility}
      setUserDetails(newProfile)
      setAllEvents(newDisponibility)
    }

    useEffect(()=>{
      if(!disponibility || disponibility=={} || Object.keys(disponibility).length === 0) {}else
      {  
          let newProfile={id:currentAccount,username:username,email:email,img:localProfileImage,background:backgroundImage,description:newDescription,disponibility:disponibility}
  ...
  <header  style={{backgroundImage:`url(${backgroundImage})`}}  className="noHover">
  </header>
    ):(  
    
 <header data-bs-toggle="modal" data-bs-target="#backgroundImage" style={{ backgroundImage: `url(${backgroundImage})` }}>
                </header>)}
                <div className="modal fade" id="backgroundImage" data-bs-backdrop="static" data-bs-keyboard="false" tabIndex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
 <div className="modal-dialog">
                      <div className="modal-content">
                        ...
                        <div class="modal-body bodyModal">
                          <img src={fileBackgroundURL} className="photoModal" alt="nessuna immagine scelta"></img>
                        </div>
                        <div class="modal-footer">
                          <button type="button" className="btnCancel" onClick={()=>setFileBackgroundURL("")}>Reset</button>
                          {!fileBackgroundURL ? (<button type="button" className="btnReview"  onClick={() => inputSfondoRef.current.click()}>Scegli foto...</button>) :
                            (<button type="button" data-bs-dismiss="modal" className="btnReview" onClick={() => confirmModifyBackgroundImage()}>Conferma</button>)}
                          <input type="file" className="d-none" ref={inputSfondoRef} onChangeCapture={onFileBackgroundChangeCapture}></input>
                        </div>
                      </div>
                    </div>
                  </div>
  ...
  <main>
    <div className="row">
      ...
      {
                  currentAccount==id.userid?(
          <img className="photo" src={localProfileImage} data-bs-toggle="modal" data-bs-target="#staticBackdrop" />):
          ...
          <div className="modal-dialog">
            <div className="modal-content">
              ...
              <div class="modal-body bodyModal">
                <img src={fileImageURL} className="photoModal" alt="nessuna immagine scelta" ></img>
              </div>
              <div class="modal-footer">
                <button type="button" className="btnCancel"  onClick={()=>setFileImageURL("")}>Reset</button>
                {!fileImageURL?(<button type="button" className="btnReview" onClick={()=>inputImageRef.current.click()}>Scegli foto...</button>):
                (<button type="button" className="btnReview " data-bs-dismiss="modal" onClick={()=>confirmModifyProfileImage()}>Conferma</button>)}
            ...
            <div className="modal-dialog">
              <div className="modal-content">
                ...
                <div class="modal-footer">
                  ...
                  <button type="button" className="btnReview" data-bs-dismiss="modal" onClick={()=>confirmModifyUsername()}>Conferma</button>
                </div>
              </div>
            </div>
          </div>
          ...
          <p data-bs-toggle="modal" data-bs-target="#modalEmail" className="activeMouseHover">Email: {email} ✏ </p>
            ...
            <div className="modal-dialog">
              <div className="modal-content">
                ...
                <div class="modal-footer">
                  <button type="button" className="btn btn-secondary" data-bs-dismiss="modal">Annulla</button>
                  <button type="button" className="btnReview" data-bs-dismiss="modal" onClick={()=>confirmModifyEmail()}>Conferma</button>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="stats row">
          <div className="stats" >
            ...
            {!averageVote?(<p className="desc">Average rating: 0⭐</p>):(<p className="desc">Average rating: {averageVote}⭐</p>)}

          </div>
        </div>
            {currentAccount==id.userid?(<>
            
          <p data-bs-toggle="modal" data-bs-target="#modalDescription" className="desc activeMouseHover"> {description} ✏ </p>
            <div className="modal fade" id="modalDescription" data-bs-backdrop="static" data-bs-keyboard="false" tabIndex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
            <div className="modal-dialog">
              <div className="modal-content">
                <div className="modal-header">
                ...
                <div class="modal-body bodyModal">
                  <textarea className="form-control" value={newDescription} onChange={(event) => setNewDescription(event.currentTarget.value)} />
                </div>
                <div class="modal-footer">
                  <button type="button" className="btn btn-secondary" data-bs-dismiss="modal">Annulla</button>
                  <button type="button" className="btnReview" data-bs-dismiss="modal" onClick={()=>confirmModifyDescription()}>Conferma</button>
                </div>
              </div>
            </div>
          </div>
        ...
        <TabPanel value={value} index={1}>
                    {
                      currentAccount==id.userid?(<></>):(
                      presentReview==true?(<>
                      <button data-bs-toggle="modal" className="btnReview" data-bs-target="#modalRecensione">Modifica recensione</button>
                      ...
                      (<button className="btnReview" data-bs-toggle="modal" data-bs-target="#modalRecensione">Lascia una recensione...</button>)
                      )         
                    }
                      <div className="modal fade" id="modalRecensione" data-bs-backdrop="static" data-bs-keyboard="false" tabIndex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                        <div className="modal-dialog">
                          <div className="modal-content">
                            ...
                            <input type="text" size={30} placeholder="Titolo recensione..." maxLength={30} value={title} onChange={(event) => setTitle(event.currentTarget.value)} />
                              <textarea className="review "type="text" placeholder="Uno splendido insegnante, durante il corso..." value={text} onChange={(event) => setText(event.currentTarget.value)} />
                              <Rating
                            ...
                            <div class="modal-footer">
                              <button type="button" className="btnCancel" data-bs-dismiss="modal">Annulla</button>
                              {presentReview==true?
                              (<button type="button" className="btnReview" data-bs-dismiss="modal" onClick={()=>modifyReview()}>Modifica recensione</button>)
                              :(<button type="button" className="btnReview" data-bs-dismiss="modal" onClick={()=>saveData()}>Invia recensione</button>)}
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="modal fade" id="modalEliminaRecensione" data-bs-backdrop="static" data-bs-keyboard="false" tabIndex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                        <div className="modal-dialog">
                          <div className="modal-content">
                            ...
                            <div class="modal-footer">
                              <button type="button" className="btnCancel" data-bs-dismiss="modal">Annulla</button>
                              <button type="button" className="btnDeliteReview" data-bs-dismiss="modal" onClick={()=>deleteReview()}>Elimina recensione</button>
                            </div>
                          </div>
                        </div>
                      </div>
                      { reviews.map((review) => {
                            if(review)  return <Review key={review.content.id} item={review.content}/>
                            
                      })}
                  </TabPanel>
                  <TabPanel value={value} index={2}>
                  <button ref={clickBooking} data-bs-toggle="modal" data-bs-target="#prenotazione" className="invisibile"></button>
                  <div className="modal fade" id="prenotazione" data-bs-backdrop="static" data-bs-keyboard="false" tabIndex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                      ...
                      <div className="modal-content">
                        ...
                        <div class="modal-body bodyModal">
                          <p>Sei sicuro di voler prenotare questo slot  {`${event.start}`} to {`${event.end}`} ?</p>
                        </div>
                        <div class="modal-footer">
                          <button type="button" className="btnCancel" data-bs-dismiss="modal">Annulla</button>
                            <button type="button" data-bs-dismiss="modal" className="btnReview" onClick={() => confirmBooking()}>Conferma</button>
                        </div>
                      </div>
                    </div>
                  </div>

                  <button ref={clickRemove} data-bs-toggle="modal" data-bs-target="#removeBooking" className="invisibile"></button>
                  <div className="modal fade" id="removeBooking" data-bs-backdrop="static" data-bs-keyboard="false" tabIndex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                          <div className="modal-dialog">
                      <div className="modal-content">
                        <div className="modal-header">
                        ...
                        <div class="modal-body bodyModal">
                          <p>Sei sicuro di voler rimuove la prenotazione allo slot  {`${event.start}`} to {`${event.end}`} ?</p>
                        </div>
                        <div class="modal-footer">
                          <button type="button" className="btnCancel" data-bs-dismiss="modal">Annulla</button>
                            <button type="button" data-bs-dismiss="modal" className="btnReview" onClick={() => removeBooking()}>Conferma</button>
                        </div>
                      </div>
                    </div>
                  </div>

                  <button ref={clickRemoveDisponibility} data-bs-toggle="modal" data-bs-target="#removeDisponibility" className="invisibile"></button>
                  <div className="modal fade" id="removeDisponibility" data-bs-backdrop="static" data-bs-keyboard="false" tabIndex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                          <div className="modal-dialog">
                      <div className="modal-content">
                        <div className="modal-header">
                          <h5 className="modal-title" id="staticBackdropLabel">Rimozione prenotazione</h5>
                        ...
                        <div class="modal-body bodyModal">
                          <p>Sei sicuro di voler rimuove la disponibilità allo slot  {`${event.start}`} to {`${event.end}`} ?</p>
                        </div>
                        <div class="modal-footer">
                          <button type="button" className="btnCancel" data-bs-dismiss="modal">Annulla</button>
                            <button type="button" data-bs-dismiss="modal" className="btnReview" onClick={() => removeDisponibility()}>Conferma</button>
                        </div>
                      </div>
                    </div>
                  </div>
                      ...
                      <Accordion.Collapse eventKey="0">
                        <Card.Body>
                          <div className="containerDate">
                            ...
                            timeCaption="time"
                            dateFormat="dd, yyyy h:mm aa"/>
                            <button onClick={()=>insertDate()}><p>+</p></button>
                          </div>
...

                                {
                                    newDate.map((data)=>{
                                      if(!data || !data.start) return
                                        return <><tr><td><p>From: {`${data.start}`}</p></td><td rowSpan={2}><button onClick={() => removeDate(data.id)}><p>X</p></button></td></tr><tr><td><p>To: {`${data.start}`}</p></td></tr></>       

                                      })
                                }
                                 </table> 
                                <button className="btnInsertDate" onClick={()=>insertDates()}>Inserisci date</button>
